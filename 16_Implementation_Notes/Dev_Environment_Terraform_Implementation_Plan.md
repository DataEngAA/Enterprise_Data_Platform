...
**Bootstrap Update 2 (the trust-policy addition) remains the next implementation stage after the bootstrap IAM validation sequence above completes — it is not yet unblocked.**
...
## 29. User Data / Bootstrap Script Strategy and Scope — FINALIZED (2026-07-26)
...
- `jq`.
- `unzip`.
- Common shell utilities reasonably needed for day-to-day development (e.g., `curl`, `tar`, `less` — exact list finalized when the script is actually authored, but bounded to this same "prerequisite tooling" category, not open-ended).
- Python tooling prerequisites (whatever `uv` itself needs present, if anything, on a fresh AL2023 image).
- `uv` (Python package/dependency manager, §31).
- Project working directories (e.g., creating an empty `~/projects` or equivalent directory structure for the developer to later `git clone` into — **the script does not perform the clone itself**, §29.2).
